...
Mod::Mod(fs::path modDir, char* jsonBuf)
{
	...
	{
		Log::Error(
			"Failed reading mod file %s: encountered parse error \"%s\" at offset %s",
	...
	if (!modJson.IsObject())
	{
		Log::Error("Failed reading mod file %s: file is not a JSON object", (modDir / "mod.json").string().c_str());
		return;
	}
	...
	{
		Log::Error("Failed reading mod file %s: missing required member \"Name\"", (modDir / "mod.json").string().c_str());
		return;
	}
	...
	{
		Version = "0.0.0";
		Log::Warn("Mod file %s is missing a version, consider adding a version", (modDir / "mod.json").string().c_str());
	}
	...
	{
		Log::Info("Mod file %s is missing a LoadPriority, consider adding one", (modDir / "mod.json").string().c_str());
		LoadPriority = 0;
	}
...
void ModManager::LoadMods()
{
	...
	std::ifstream enabledModsStream(GetSpectrePrefix() + "/enabledmods.json");
	...
	for (fs::path modDir : modDirs)
	{
		...
		if (jsonStream.fail())
		{
			Log::Warn("Mod %s has a directory but no mod.json", modDir.string().c_str());
			continue;
		}
		...
		{
			Log::Info("Loaded mod %s successfully", mod.Name.c_str());
			if (mod.Enabled)
				Log::Info("Mod %s is enabled", mod.Name.c_str());
			else
				Log::Info("Mod %s is disabled", mod.Name.c_str());

			m_loadedMods.push_back(mod);
		}
		else
			Log::Warn("Skipping loading mod file %s", (modDir / "mod.json").string().c_str());
	}
		...
		if (fs::exists(mod.ModDirectory / "audio"))
		{
			for (fs::directory_entry file : fs::directory_iterator(mod.ModDirectory / "audio"))
			{
				if (fs::is_regular_file(file) && file.path().extension().string() == ".json")
				{
					...
					{
						Log::Warn("Mod %s has an invalid audio def %s", mod.Name.c_str(), file.path().filename().string().c_str());
						continue;
					}
				}
			}
...
void ModManager::UnloadMods()
{
	...
	std::ofstream writeStream(GetSpectrePrefix() + "/enabledmods.json");
...
{
	return fs::path(GetSpectrePrefix() + MOD_FOLDER_SUFFIX);
}
fs::path GetCompiledAssetsPath()
{
	return fs::path(GetSpectrePrefix() + COMPILED_ASSETS_SUFFIX);
}