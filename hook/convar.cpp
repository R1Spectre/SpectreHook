...
void InitialiseConVars(HMODULE baseAddress)
{
	...
	conVarRegister = (ConVarRegisterType)((char*)baseAddress + 0x481AF0);

	g_pConVar_Vtable = (char*)baseAddress + 0x697A48;
	g_pIConVar_Vtable = (char*)baseAddress + 0x697908;
...
{
	Log::Info("Registering Convar %s", pszName);
...
void ConVar::SetValue(const char* pszValue)
{
	...
	if (!SetColorFromString(pszValue))
	{
		...
		{
			Log::Warn("Warning: ConVar '%s' = '%s' is infinite, clamping value.\n", GetBaseName(), pszValue);
			flNewValue = FLT_MAX;
		}
