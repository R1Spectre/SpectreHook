...
void InitialiseTier0GameUtilFunctions(HMODULE baseAddress)
{
	if (!baseAddress)
	{
		Log::Critical("tier0 base address is null, but it should be already loaded");
	...
	{
		Log::Critical("Address of g_pMemAllocSingleton is a null pointer, this should never happen");
	...
	if (!*ppMemAllocSingleton)
	{
		...
		*ppMemAllocSingleton = g_pMemAllocSingleton;
		Log::Info("Created new g_pMemAllocSingleton");
	}
	else
	{
