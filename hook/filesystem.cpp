...
void InitialiseFilesystem(HMODULE baseAddress)
{
	...
	// create hooks
	HookEnabler hook;
	ENABLER_CREATEHOOK(hook, (char*)baseAddress + 0x6A420, &ReadFileFromVPKHook, reinterpret_cast<LPVOID*>(&readFileFromVPK));
...
void SetNewModSearchPaths(Mod* mod)
{
	...
	if (mod != nullptr)
	{
		...
		{
			Log::Info("Changing mod search path from \"%s\" to \"%s\"", currentModPath.c_str(), mod->ModDirectory.string().c_str());
...
bool TryReplaceFile(char* path, bool shouldCompile)
{
	...
	{
		Log::Info("File \"%s\" replaced by \"%s\"", path, file->second.owningMod->Name.c_str());
...
void AddSearchPathHook(IFileSystem* fileSystem, const char* pPath, const char* pathID, SearchPathAdd_t addType)
{
	...
	Log::Info("AddSearchPath: \"%s\", \"%s\", %d", pPath, pathID, addType);
